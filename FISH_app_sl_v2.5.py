...
class FISHDataAnalyzer:
    def __init__(self, fish_data, cell_counts, selected_patterns, probe_name, calculation_method, log_callback=None):
        ...
        self.selected_patterns = selected_patterns # List of short pattern names in user-selected order
    ...
    def reorganize_data_with_techs(self, total_cells):
        ...
        if self.fish_data.empty:
            self.log_message(f"Input fish_data is empty for {total_cells} cells. Cannot reorganize.", "red", True)
            return pd.DataFrame()
        if 'Signal_Pattern' not in self.fish_data.columns:
            self.log_message("Critical Error: 'Signal_Pattern' column missing. Cannot map patterns.", "red", True)
            return pd.DataFrame()
        ...
        if 'Tech No.' not in df.columns:
            self.log_message("Critical Error: 'Tech No.' column missing. Cannot filter by Tech.", "red", True)
            df_filtered = df # Proceed with unfiltered, might lead to other issues or empty results
        else:
            df_filtered = df[df['Tech No.'].isin(['Tech 1', 'Tech 2'])]

        if df_filtered.empty and 'Tech No.' in df.columns :
             self.log_message(f"Warning: Filtering by 'Tech 1' or 'Tech 2' resulted in an empty dataset for {total_cells} cells.", "red", True)

        return self.process_cell_count(df_filtered, total_cells)

    def process_cell_count(self, df, total_cells): # df here is df_filtered
        if df.empty:
            self.log_message(f"Cannot process cell count for {total_cells} as the filtered data is empty.", "red", True)
            return pd.DataFrame(columns=['Case No.', 'Hybe date', 'Case ID', '# Scored'])

        if 'Score No.' not in df.columns:
            self.log_message(f"Critical Error: 'Score No.' column missing for {total_cells} cells.", "red", True)
            return pd.DataFrame(columns=['Case No.', 'Hybe date', 'Case ID', '# Scored'])

        df_cells = pd.DataFrame() # Initialize to empty
        if total_cells == self.cell_counts[0]: # First count
            if total_cells == 100:
                df_cells = df[df['Score No.'] == 50].copy()
            elif total_cells == 200:
                df_cells = df[df['Score No.'] == 100].copy()
            else:
                self.log_message(f"Warning: Unexpected 'total_cells' ({total_cells}) for first count. Check config.", "red", True)
        else: # Second count (total_cells == self.cell_counts[1])
            if total_cells == 200:
                df_cells = df[df['Score No.'] == 100].copy()
            elif total_cells == 500:
                df_cells = df[df['Score No.'].isin([100, 150])].copy()
            else:
                self.log_message(f"Warning: Unexpected 'total_cells' ({total_cells}) for second count. Check config.", "red", True)
        
        if df_cells.empty:
            self.log_message(f"No data matched score criteria for {total_cells} cells.", "red", True)
            # Return df with expected columns for consistency, even if empty
            result_df = pd.DataFrame(columns=['Case No.', 'Hybe date', 'Case ID', '# Scored'])
            result_df['# Scored'] = total_cells
            return result_df

        if 'Signal_Pattern_Short' not in df_cells.columns:
            self.log_message(f"Critical Error: 'Signal_Pattern_Short' missing after filtering for {total_cells} cells.", "red", True)
            return pd.DataFrame(columns=['Case No.', 'Hybe date', 'Case ID', '# Scored'])

        all_patterns_short_in_df_cells = df_cells['Signal_Pattern_Short'].unique()
        patterns_to_process = [p for p in self.selected_patterns if p in all_patterns_short_in_df_cells]
        
        self.log_message(f"Will process {len(patterns_to_process)} patterns for {total_cells} cells.")
        if not patterns_to_process and self.selected_patterns:
            self.log_message(f"Warning: None of {len(self.selected_patterns)} user-selected patterns found in data for {total_cells} cells.", "red", True)

        required_case_info_cols = ['Case No.', 'Hybe date', 'Case ID']
        if not all(col in df_cells.columns for col in required_case_info_cols):
            self.log_message(f"Critical Error: Case info columns missing ({required_case_info_cols}) for {total_cells} cells.", "red", True)
            return pd.DataFrame(columns=['Case No.', 'Hybe date', 'Case ID', '# Scored'])

        case_info_df = df_cells[required_case_info_cols].drop_duplicates(subset=['Case No.'])
        ...
        if 'Value' not in df_cells.columns:
            self.log_message(f"Critical Error: 'Value' column missing for pattern processing ({total_cells} cells). Pattern values will be 0.", "red", True)

        for pattern in patterns_to_process:
            tech1_col, tech2_col, pct_col = f"{pattern}_Tech 1", f"{pattern}_Tech 2", f"{pattern}_%"
            result_df[tech1_col], result_df[tech2_col], result_df[pct_col] = 0.0, 0.0, 0.0 # Initialize as float

            for idx, case_row in result_df.iterrows():
                case_no_current = case_row['Case No.']
                tech1_value, tech2_value = 0.0, 0.0 # Default to float

                if 'Value' in df_cells.columns:
                ...
                pct_value = (sum_value / total_cells) * 100.0 if total_cells > 0 else 0.0

                result_df.loc[idx, tech1_col] = float(tech1_value)
                result_df.loc[idx, tech2_col] = float(tech2_value)
                result_df.loc[idx, pct_col] = float(pct_value)
        
        frg3_patterns_processed = [p for p in patterns_to_process if 'FRG>3swa' in p or 'FRW>3swa' in p]
        frg1_3_patterns_processed = [p for p in patterns_to_process if any(substr in p for substr in ['FRG>1swa<3swa', 'FRW>1swa<3swa', 'FRG>1<3swa', 'FRW>1<3swa'])]

        if frg3_patterns_processed and frg1_3_patterns_processed:
            self.log_message("Creating combined FRG>1swa pattern (Break Apart)...", "blue")
            frg3_p_name = frg3_patterns_processed[0]
            frg1_3_p_name = frg1_3_patterns_processed[0]

            combined_tech1_col, combined_tech2_col, combined_pct_col = "FRG>1swa_Tech 1", "FRG>1swa_Tech 2", "FRG>1swa_%"
            
            val_frg3_t1 = result_df.get(f"{frg3_p_name}_Tech 1", pd.Series(0.0, index=result_df.index, dtype=float)) if f"{frg3_p_name}_Tech 1" in result_df else pd.Series(0.0, index=result_df.index, dtype=float)
            val_frg1_3_t1 = result_df.get(f"{frg1_3_p_name}_Tech 1", pd.Series(0.0, index=result_df.index, dtype=float)) if f"{frg1_3_p_name}_Tech 1" in result_df else pd.Series(0.0, index=result_df.index, dtype=float)
            result_df[combined_tech1_col] = val_frg3_t1 + val_frg1_3_t1

            val_frg3_t2 = result_df.get(f"{frg3_p_name}_Tech 2", pd.Series(0.0, index=result_df.index, dtype=float)) if f"{frg3_p_name}_Tech 2" in result_df else pd.Series(0.0, index=result_df.index, dtype=float)
            val_frg1_3_t2 = result_df.get(f"{frg1_3_p_name}_Tech 2", pd.Series(0.0, index=result_df.index, dtype=float)) if f"{frg1_3_p_name}_Tech 2" in result_df else pd.Series(0.0, index=result_df.index, dtype=float)
            ...
            result_df[combined_pct_col] = (combined_sum_values / total_cells) * 100.0 if total_cells > 0 else 0.0
        return result_df

    def _calculate_single_cutoff_set(self, df, total_cells, calculation_method_name, stat_func_ppf, p_adjust_func=None):
        """Helper function for cutoff calculations."""
        self.log_message(f"Calculating cutoff values ({calculation_method_name} method) for {total_cells} cells...")
        if df.empty:
            self.log_message(f"Skipping {calculation_method_name} cutoff for {total_cells} cells: input data empty.", "red", True)
            return pd.DataFrame()

        all_patterns_in_df = list(set(col.split('_')[0] for col in df.columns if col.endswith('_Tech 1')))
        patterns_for_cutoff_report = [p for p in self.selected_patterns if p in all_patterns_in_df]
        
        frg3_user = any('FRG>3swa' in sp or 'FRW>3swa' in sp for sp in self.selected_patterns)
        frg1_3_user = any(any(substr in sp for substr in ['FRG>1swa<3swa', 'FRW>1swa<3swa', 'FRG>1<3swa', 'FRW>1<3swa']) for sp in self.selected_patterns)
        if frg3_user and frg1_3_user and "FRG>1swa" in all_patterns_in_df and "FRG>1swa" not in patterns_for_cutoff_report:
            patterns_for_cutoff_report.append("FRG>1swa")
        
        results = []
        confidence_level = 0.95

        for pattern in patterns_for_cutoff_report:
            tech1_col, tech2_col, pct_col = f"{pattern}_Tech 1", f"{pattern}_Tech 2", f"{pattern}_%"
            if not (tech1_col in df.columns and tech2_col in df.columns and pct_col in df.columns):
                self.log_message(f"Skipping {calculation_method_name} cutoff for '{pattern}': data columns missing.", "red", True)
                continue

            sum_values = df[tech1_col].fillna(0) + df[tech2_col].fillna(0)
            percentages = df[pct_col].fillna(0).values / 100.0
            
            cutoff_value_pct = 0.0
            grey_zone_lower_val = 0.0
            grey_zone_upper_val = 0.0
            std_dev_val = 0.0

            if calculation_method_name == "Beta":
                max_sum = sum_values.max()
                percentages_for_sd = percentages.copy()
                if np.all(percentages_for_sd == 0):
                    self.log_message(f"WARNING: All percentages 0 for '{pattern}'. Adding 0.5% for SD (Beta).", "red", True)
                    if len(percentages_for_sd) > 0: percentages_for_sd[0] = 0.005
                
                k_param = min(int(max_sum if pd.notna(max_sum) else 0), total_cells - 1)
                k_param = max(0, k_param)
                param_a, param_b = k_param + 1, total_cells - k_param + 1
                param_a = max(1e-9, param_a)
                param_b = max(1e-9, param_b)
                cutoff_value = stat_func_ppf(confidence_level, param_a, param_b) if k_param <= total_cells else 1.0
                cutoff_value_pct = cutoff_value * 100.0
                std_dev_val = np.std(percentages_for_sd) * 100.0
                if std_dev_val < 0.1:
                    std_dev_val = 0.5
                    self.log_message(f"Applied min SD 0.5% for '{pattern}' (Beta).", "red", True)
                grey_zone_lower_val = max(0, cutoff_value_pct - 2 * std_dev_val)
                grey_zone_upper_val = min(100, cutoff_value_pct + 2 * std_dev_val)

            elif calculation_method_name == "Cribinom":
                p_for_cutoff = np.mean(percentages)
                if np.all(percentages == 0) or p_for_cutoff == 0:
                    p_for_cutoff = 0.005
                    self.log_message(f"WARNING: All percentages 0 or mean 0 for '{pattern}'. Using p=0.005 (Cribinom).", "red", True)
                
                cutoff_count = stat_func_ppf(confidence_level, total_cells, p_for_cutoff)
                if cutoff_count == 0 and 0 < p_for_cutoff < 0.01:
                    cutoff_count = 1
                    self.log_message(f"Adjusted Cribinom cutoff for '{pattern}' to 1.", "red", True)
                cutoff_value_pct = (cutoff_count / total_cells) * 100.0
                std_dev_val = np.std(percentages) * 100.0 # Use original percentages for std dev
                if std_dev_val < 0.1 and np.any(percentages > 0): # Only if some data > 0
                    std_dev_val = 0.5
                    self.log_message(f"Applied min SD 0.5% for '{pattern}' (Cribinom).", "red", True)
                elif np.all(percentages==0): # if all are zero, std_dev is zero
                    std_dev_val = 0.0

                grey_zone_lower_val = cutoff_value_pct
                grey_zone_upper_val = min(100, cutoff_value_pct + 3.0)
            
            valid_sum_values = sum_values.values[sum_values.values >= 0]
            data_range = f"{int(min(valid_sum_values))}-{int(max(valid_sum_values))}" if len(valid_sum_values) > 0 else "0-0"
            
            results.append({
                'Signal_Pattern': pattern, 'Cutoff_95%_CI': f"{cutoff_value_pct:.2f}%",
                'Grey_Zone_Lower': f"{grey_zone_lower_val:.2f}%", 'Grey_Zone_Upper': f"{grey_zone_upper_val:.2f}%",
                'Range': data_range, 'Standard_Deviation': f"{std_dev_val:.2f}%", 'Controls': "(20 controls)"
            })
        self.log_message(f"Calculated {len(results)} {calculation_method_name} cutoff values for {total_cells} cells.")
        return pd.DataFrame(results)

    def calculate_cutoff_and_grey_zones(self, df, total_cells):
        return self._calculate_single_cutoff_set(df, total_cells, "Beta", beta.ppf)

    def calculate_cutoff_and_grey_zones_cribinom(self, df, total_cells):
        return self._calculate_single_cutoff_set(df, total_cells, "Cribinom", binom.ppf)


    def create_reorganized_html_content(self, df_first, df_second):
        table_width, font_size = "100%", "10px"
        # ... (rest of HTML structure code remains same as your v2.3) ...
        ...
        if df_first is not None and not df_first.empty:
            ...
            html_content += f'<div class="table-section"><h2>{self.probe_name} {self.cell_counts[0]} Cells Data</h2><p>No data available for {self.cell_counts[0]} cells analysis.</p></div>'
        ...
        if df_second is not None and not df_second.empty:
            ...
            html_content += f'<div class="table-section"><h2>{self.probe_name} {self.cell_counts[1]} Cells Data</h2><p>No data available for {self.cell_counts[1]} cells analysis.</p></div>'
    ...
    def create_table_content(self, df, total_cells):
        if df.empty:
            self.log_message(f"Cannot create HTML table for {total_cells} cells as input data is empty.", "red", True)
            return "<p>No data available to display in table.</p>"
            
        base_cols = ['Case No.', 'Hybe date', 'Case ID', '# Scored']
        
        all_patterns_in_data_df = list(set(col.split('_')[0] for col in df.columns if col.endswith('_Tech 1') or col.endswith('_Tech 2') or col.endswith('_')))

        final_pattern_names_for_table = [
            p for p in self.selected_patterns if p in all_patterns_in_data_df
        ]
        ...
        if frg3_user_selected and frg1_3_user_selected and "FRG>1swa" in all_patterns_in_data_df:
        ...
        if not final_pattern_names_for_table:
             self.log_message(f"No patterns to display in table for {total_cells} cells after filtering by selection and data availability.", "red", True)
        ...
        for pattern in final_pattern_names_for_table:
            ...
            if pct_col in df.columns:
                # Ensure column is numeric before max(), or handle potential errors if it's not
                if pd.api.types.is_numeric_dtype(df[pct_col]):
                    ...
                    self.log_message(f"Percentage column {pct_col} is not numeric, cannot find max for highlighting.", "red", True)

        html = "<table><thead><tr>"
        for col in base_cols: html += f'<th rowspan="2" class="header-light-blue">{col}</th>'
        for pattern_header in final_pattern_names_for_table: html += f'<th colspan="3" class="header-dark-blue">{pattern_header}</th>'
        html += '</tr><tr>'
        ...
        for row_idx, row_data in df.iterrows():
            ...
            for col_name in base_cols: html += f'<td>{row_data.get(col_name, "")}</td>'
            for pattern_data_col in final_pattern_names_for_table:
                t1_col_name, t2_col_name, pct_col_name = f"{pattern_data_col}_Tech 1", f"{pattern_data_col}_Tech 2", f"{pattern_data_col}_%"
                v1 = row_data.get(t1_col_name, 0)
                v2 = row_data.get(t2_col_name, 0)
                vp = row_data.get(pct_col_name, 0.0)
                
                # Ensure vp is float for formatting
                try:
                    vp_float = float(vp)
                except (ValueError, TypeError):
                    vp_float = 0.0 # Default if conversion fails
                
                highlight_class = ""
                if pattern_data_col in max_values and \
                   max_row_indices.get(pattern_data_col) == row_idx and \
                   pd.notna(vp_float) and vp_float == max_values.get(pattern_data_col) and vp_float > 0: # Compare with float
                     highlight_class = "highlight"

                html += f'<td class="{highlight_class}">{v1}</td><td class="{highlight_class}">{v2}</td><td class="{highlight_class}">{vp_float:.2f}%</td>'
            html += '</tr>'
        html += "</tbody></table>"
        return html

    def create_cutoff_html_content(self, cutoff_first, cutoff_second):
        # ... (HTML structure remains same as your v2.3) ...
        html_content = f"""<!DOCTYPE html><html><head>
        ...
        else: html_content += f'<div class="no-data">No cutoff data for {self.cell_counts[0]} cells</div>'
        ...
        else: html_content += f'<div class="no-data">No cutoff data for {self.cell_counts[1]} cells</div>'
...
def streamlit_log_message(message, color="black", bold=False):
    ...
    # Make "orange" (warnings) and "red" (errors) always bold and red.
    if color == "orange" or color == "red":
        styled_log_entry = f":red[**{log_entry_text}**]"
    elif bold: # For other colors, apply bold if explicitly requested
        styled_log_entry = f"**{log_entry_text}**"
        # Then apply the specific color if not black
        if color == "green": styled_log_entry = f":green[{styled_log_entry}]"
        elif color == "blue": styled_log_entry = f":blue[{styled_log_entry}]"
    # Apply color if not bold and not black/red/orange
    elif color == "green": styled_log_entry = f":green[{log_entry_text}]"
    elif color == "blue": styled_log_entry = f":blue[{log_entry_text}]"
...
def process_single_table_excel(df_raw, start, end, log_func):
    try:
        ...
        if table_with_headers.shape[0] < 2: 
            log_func(f"Table at original rows {start+1}-{end+1} is too short for header processing.", "red", True)
            return []
        ...
        if len(final_headers) != table_data_rows.shape[1]:
            log_func(f"Header length mismatch in table at original rows {start+1}-{end+1}. Adjusting.", "red", True)
        ...
        if "Tech No." in final_headers:
            ...
            log_func(f"Warning: 'Tech No.' column not found in table at original rows {start+1}-{end+1}. Cannot process tech-specific data.", "red", True)
        return processed_data_list
    except Exception as e:
        log_func(f"CRITICAL error processing table at original rows {start+1}-{end+1}: {e}", "red", True)
        import traceback
        log_func(traceback.format_exc(),"red", True) 
        return []
...
def is_valid_table_excel(df_raw, start, end, log_func):
    try:
        ...
        log_func(f"Error during table validation (rows {start+1}-{end+1}): {e}", "red", True)
        return True
    ...
    try:
        ...
        for i, (start_idx, end_idx) in enumerate(zip(table_starts, table_ends)):
            ...
            if start_idx >= len(df_raw) or end_idx >= len(df_raw) or start_idx > end_idx:
                log_func(f"Skipping invalid table indices: start={start_idx}, end={end_idx}", "red", True)
                excluded_count += 1
                continue
            ...
            if is_valid_table_excel(df_raw, start_idx, end_idx, log_func):
                ...
                excluded_count += 1
                log_func(f"Table {i+1} (original rows {start_idx+1}-{end_idx+1}) excluded due to insufficient valid data.", "red", True)
        
        if excluded_count > 0:
            log_func(f"INFO: Excluded {excluded_count} tables due to insufficient data.", "blue")
        ...
        if all_tables_data_list:
            ...
            log_func("No valid data tables could be extracted from the Excel file. Please check file format and content.", "red", True)
            return pd.DataFrame()
    except Exception as e:
...
def detect_cell_counts_streamlit(fish_data_df, log_func):
    if fish_data_df is None or fish_data_df.empty:
        log_func("FISH data is empty, defaulting cell counts to [200, 500].", "red", True)
        return [200, 500]
    
    if 'Score No.' not in fish_data_df.columns:
        log_func("'Score No.' column not found in FISH data. Defaulting cell counts.", "red", True)
        return [200, 500]
        
    score_nos = fish_data_df['Score No.'].dropna().unique()
    log_func(f"Found Score No. values in data: {sorted(list(score_nos))}")
    
    counts = [200, 500] 
    float_score_nos = set()
    for s in score_nos:
        try:
            float_score_nos.add(float(s))
        except (ValueError, TypeError):
            log_func(f"Could not convert Score No. '{s}' to float. Skipping.", "red", True)


    if 50.0 in float_score_nos: 
        counts = [100, 200] 
    elif 100.0 in float_score_nos and 150.0 in float_score_nos: 
        counts = [200, 500] 
    elif 100.0 in float_score_nos: 
        counts = [200, 500] 
    else:
        log_func(f"Could not confidently determine cell count scheme from Score Nos: {float_score_nos}. Defaulting to {counts}.", "red", True)

    log_func(f"Determined cell counts for processing: {counts}")
    return counts

def get_available_patterns_streamlit(fish_data_df, log_func):
    if fish_data_df is None or fish_data_df.empty: return []
    if 'Signal_Pattern' not in fish_data_df.columns:
        log_func("'Signal_Pattern' column not found. Cannot extract patterns.", "red", True)
        return []
    unique_patterns_raw = fish_data_df['Signal_Pattern'].dropna().unique()
    pattern_map = {p_raw: str(p_raw).split('_')[-1] if '_' in str(p_raw) else str(p_raw) for p_raw in unique_patterns_raw}
    available_short_patterns = sorted(list(set(pattern_map.values()))) # Keep sorted for consistent option display
    log_func(f"Available short signal patterns from data: {available_short_patterns}")
...
def run_fish_analysis_app():
    ...
    default_session_state = {
        ...
        'selected_patterns_ui': [], # Canonical list of selected patterns (maintains user order)
        'pattern_multiselect_widget_state': [], # Stores the direct state of the multiselect widget
    ...
    # --- Callback for common actions after selection changes ---
    def handle_selection_change_common_logic():
    ...
    # --- Callback for multiselect widget ---
    def sync_selected_patterns_from_widget():
        # Update the canonical list from the widget's current state
        st.session_state.selected_patterns_ui = st.session_state.pattern_multiselect_widget_key
        handle_selection_change_common_logic()
    ...
    if uploaded_file:
        if uploaded_file.name != st.session_state.last_uploaded_filename:
            streamlit_log_message(f"New file uploaded: {uploaded_file.name}. Resetting state.", "blue")
            st.session_state.last_uploaded_filename = uploaded_file.name
            # Reset all relevant states
            for key_to_reset in ['data_loaded_successfully', 'fish_data', 'available_patterns',
                                 'selected_patterns_ui', 'pattern_multiselect_widget_state',
                                 'html_reorganized_content', 'fname_reorganized_report',
                                 'html_cutoff_content', 'fname_cutoff_report', 'reports_generated_once']:
                if key_to_reset in default_session_state:
                     st.session_state[key_to_reset] = default_session_state[key_to_reset]
                elif hasattr(st.session_state, key_to_reset): # for safety if not in defaults
                     delattr(st.session_state, key_to_reset)
            ...
            st.rerun() # Rerun to ensure UI resets properly after new file
    ...
    if st.button("Load and Process Data", key="load_data_main_button"):
        ...
        for key_to_reset in ['html_reorganized_content', 'fname_reorganized_report',
                             'html_cutoff_content', 'fname_cutoff_report', 'reports_generated_once',
                             'selected_patterns_ui', 'pattern_multiselect_widget_state']:
            if key_to_reset in default_session_state:
                 st.session_state[key_to_reset] = default_session_state[key_to_reset]
            elif hasattr(st.session_state, key_to_reset):
                 delattr(st.session_state, key_to_reset)
        ...
        if not uploaded_file:
            ...
            # ... (rest of load and process data logic) ...
            ...
            try:
                ...
                if st.session_state.fish_data is not None and not st.session_state.fish_data.empty:
                    ...
                    if not st.session_state.available_patterns:
                        ...
                        streamlit_log_message("Warning: No signal patterns identified in data.", "red", True)
                else:
                ...
                if 'load_progress_bar' in locals() and hasattr(load_progress_bar, 'empty'):
    ...
    if st.session_state.data_loaded_successfully and st.session_state.available_patterns:
        ...
        if col1_btn.button("Select All Patterns", key="select_all_btn"):
            ...
            st.session_state.pattern_multiselect_widget_state = st.session_state.selected_patterns_ui[:] # Sync widget state
            handle_selection_change_common_logic()
            st.rerun()

        if col2_btn.button("Clear All Selections", key="clear_all_btn"):
            st.session_state.selected_patterns_ui = []
            st.session_state.pattern_multiselect_widget_state = [] # Sync widget state
            handle_selection_change_common_logic()
            st.rerun()
        
        st.write("Select Signal Patterns (patterns will appear in reports in the order selected):")
        
        # Use the 'pattern_multiselect_widget_state' for the multiselect's internal value via key
        # and 'selected_patterns_ui' for the default.
        # The on_change callback will sync pattern_multiselect_widget_state to selected_patterns_ui.
        st.multiselect(
            "Available patterns (select in desired order):",
            options=st.session_state.available_patterns, # Options are always all available
            default=st.session_state.selected_patterns_ui, # Default to the canonical list
            key="pattern_multiselect_widget_key", # Key for the widget state
            on_change=sync_selected_patterns_from_widget # Callback updates selected_patterns_ui
        )
        
        # Display current selection order from the canonical list
        if st.session_state.selected_patterns_ui:
            st.write("Current selection order for report:")
            for i, pattern in enumerate(st.session_state.selected_patterns_ui):
        ...
        streamlit_log_message("Warning: Data loaded, but no signal patterns found for selection.", "red", True)
    else:
    ...
    if st.session_state.data_loaded_successfully and st.session_state.available_patterns:
        # Check the canonical selected_patterns_ui for the warning
        if not st.session_state.selected_patterns_ui:
            ...
            # ... (rest of Step 3 logic for report generation and download buttons) ...
                ...
                try:
                    with st.spinner("Generating reports... This may take a few moments."):
                        analyzer = FISHDataAnalyzer(
                            ...
                            st.session_state.selected_patterns_ui, # Use the canonical list
                            st.session_state.probe_name,
                            calc_method_actual, streamlit_log_message
                        )
                        
                        report_progress_bar.progress(0.1, text="Generating reorganized data (1/2)...")
                        df_first_reorg = analyzer.reorganize_data_with_techs(analyzer.cell_counts[0])
                        
                        report_progress_bar.progress(0.25, text="Generating reorganized data (2/2)...")
                        df_second_reorg = analyzer.reorganize_data_with_techs(analyzer.cell_counts[1])
                        
                        report_progress_bar.progress(0.4, text="Creating HTML for reorganized data...")
                        ...
                        report_progress_bar.progress(0.6, text="Calculating cutoff values (1/2)...")
                        ...
                        report_progress_bar.progress(0.75, text="Calculating cutoff values (2/2)...")
                        if calc_method_actual == "Beta":
                        ...
                        report_progress_bar.progress(0.9, text="Creating HTML for cutoff values...")
                    ...
                    import traceback
                    streamlit_log_message(traceback.format_exc(),"red", True)
                    st.session_state.reports_generated_once = False
                finally:
                   if 'report_progress_bar' in locals() and hasattr(report_progress_bar, 'empty'):
                       report_progress_bar.empty()
                ...
                if st.session_state.get('html_reorganized_content') and st.session_state.get('fname_reorganized_report'):
                    with dl_col1:
                        st.download_button(
                            ...
                            mime="text/html",
                            key=f"dl_reorg_{st.session_state.fname_reorganized_report}" 
                        )
                if st.session_state.get('html_cutoff_content') and st.session_state.get('fname_cutoff_report'):
                    with dl_col2:
                        st.download_button(
                            ...
                            mime="text/html",
                            key=f"dl_cutoff_{st.session_state.fname_cutoff_report}"
                        )
    elif st.session_state.data_loaded_successfully:
        st.warning("Data loaded, but no signal patterns found. Cannot generate reports.")
        streamlit_log_message("Warning: Data loaded, but no signal patterns found for report generation.", "red", True)
    else:
        st.info("Load data and select patterns in Steps 1 & 2 to enable report generation.")

    # Update log display
    log_output = "\n\n".join(st.session_state.log_messages) 
    log_placeholder.markdown(log_output if log_output else "_Log is clear._", unsafe_allow_html=False) 
